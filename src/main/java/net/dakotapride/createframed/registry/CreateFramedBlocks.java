...
public enum CreateFramedBlocks {
    ...
    CreateFramedBlocks(Supplier<CTSpriteShiftEntry> basic_ctshift,
		...
		cardboard_block = REGISTRATE.get().block(name + "_cardboard_block", CardboardBlock::new).initialProperties(() -> Blocks.MUSHROOM_STEM).properties(p -> p.sound(SoundType.CHISELED_BOOKSHELF).ignitedByLava().mapColor(template_block.defaultMapColor())).item(CardboardBlockItem::new).build().register();
		bound_cardboard_block = REGISTRATE.get().block("bound_" + name + "_cardboard_block", CardboardBlock::new).initialProperties(() -> Blocks.MUSHROOM_STEM).properties(p -> p.sound(SoundType.CHISELED_BOOKSHELF).ignitedByLava().mapColor(template_block.defaultMapColor())).item(CardboardBlockItem::new).build().register();
    }
